                    ...
                    {filteredTeam.map((member) => (
                      <div key={member.id} className="p-4 space-y-3">
                        <div className="flex items-center gap-3">
                          {member.image_url && (
                            <img className="h-10 w-10 rounded-full object-cover flex-shrink-0" src={member.image_url} alt="" />
                          )}
                          <div className="min-w-0">
                            <div className="text-sm font-medium text-gray-900 truncate">{stripHtmlTags(member.name)}</div>
                            <div className="text-xs text-gray-500">{member.position}</div>
                            <div className="text-xs text-gray-400">/{member.slug}</div>
                          </div>
                        </div>
                        <div className="flex flex-wrap items-center gap-2 text-xs text-gray-500">
                          <span className={`px-2 py-0.5 font-medium rounded-full ${member.published ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                            {member.published ? 'Published' : 'Draft'}
                          </span>
                          <span className={`px-2 py-0.5 font-medium rounded-full ${member.featured ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}`}>
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {filteredTeam.map((member) => (
                          <tr key={member.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <span className={`px-2 py-1 text-xs font-medium rounded-full ${member.published ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                                {member.published ? 'Published' : 'Draft'}
                              </span>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <span className={`px-2 py-1 text-xs font-medium rounded-full ${member.featured ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}`}>
                                {member.featured ? 'Featured' : 'Normal'}
                              </span>
                            </td>
